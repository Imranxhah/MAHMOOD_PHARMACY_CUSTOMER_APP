...
class _OTPScreenState extends State<OTPScreen> {
  ...
  Future<void> _resendOtp() async {
    if (_isResending || _countdown > 0) return; // Prevent multiple resend requests
    ...
    if (result['success']) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(result['message'] ?? AppStrings.otpResendFailed)),
      );
    }
  }

  Future<void> _verifyOtp() async {
    if (_formKey.currentState!.validate() && !_isLoading) { // Add !isLoading check
      ...
      if (result['success']) {
        ...
        if (authProvider.authStatus == AuthStatus.authenticated) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(result['message'] ?? AppStrings.verifySuccessLoggedIn)),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(result['message'] ?? AppStrings.verifySuccessLoggingIn)),
          );
          final loginResult = await authProvider.login(widget.email, widget.password!);
          if (!mounted) return;
          ...
          if (loginResult == 'success') {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text(AppStrings.verifySuccessAutoLoginFailed)),
            );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(result['message'] ?? AppStrings.verifySuccessPleaseLogin)),
          );
          Navigator.of(context).pop(); // Go back to the login screen
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(result['message'] ?? AppStrings.otpVerifyFailed)),
        );
      }
    }
  }
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                validator: (value) =>
                    (value == null || value.length != 6) ? AppStrings.enterValidOtp : null,
              ),
              const SizedBox(height: AppSizes.p20),
              ...
              TextButton(
                onPressed: _countdown == 0 && !_isResending ? _resendOtp : null, // Disable if resending or countdown active
                child: Text(_countdown > 0 ? '${AppStrings.resendOtpIn}$_countdown s' : AppStrings.resendOtp),
              ),
            ],
          ),
        ),
